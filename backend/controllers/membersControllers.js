...
export const borrowBook = async (req, res) => {
    ...
    try{
        ...
        const borrowedBook = await BorrowedBooks.findById(borrowed._id).populate('bookID')
...
export const getLoans = async (req, res) => {
    ...
    try{
        const currentLoans = await BorrowedBooks.find({userID}).populate('bookID')
...
export const renewBook = async (req, res) => {
    ...
    try{    
        await BorrowedBooks.updateOne({_id:borrowedbookID},  {expireAt: Date.now()}, { new: 'true' }).populate('bookID')
        const updated = await BorrowedBooks.findById(borrowedbookID).populate('bookID')
